...
export class Completer extends Widget {
  ...
  selectActive(): void {
    ...
    this._editor.setOption('mode', active.getAttribute('data-language-value') as string);
    this._selected.emit(active.getAttribute('data-replace-value') as string);
    this.reset();
  }
  ...
  private _evtMousedown(event: MouseEvent) {
    ...
    while (target !== document.documentElement) {
      ...
      if (target.classList.contains(ITEM_CLASS) )  {
        ...
        event.stopImmediatePropagation();
        // @ts-ignore 'mode' not in options as of release
        this._editor.setOption('mode', target.getAttribute('data-language-value') as string);
        this._selected.emit(target.getAttribute('data-replace-value') as string);
        this.reset();
        return;
      }
...
export namespace Completer {
  ...
  /**
   * A type map that may add replace text to completer matches.
   */
  export type ReplaceMap = { [index: string]: string | null };

  /**
   * A type map that may add language selection to completer matches.
   */
  export type LanguageMap = { [index: string]: string | null };

  /**
   * A type map that may add custom sort to completer matches.
   */
  ...
  export interface IItem {
    ...
    /**
     * The replaceText of a visible completer item.
     */
    replaceText: string;
  ...
  export class Renderer implements IRenderer {
    ...
    createItemNode(
      ...
      li.setAttribute('data-replace-value', item.replaceText);
      li.setAttribute('data-language-value', item.language);
      ...
      if (!JSONExt.deepEqual(typeMap, {})) {
        ...
        switch (type) {
          case 'extract':
            typeNode.textContent = 'e';
            typeNode.setAttribute(`data-color-index`, '1');
            break;
          case 'transform':
            typeNode.textContent = 't';
            typeNode.setAttribute(`data-color-index`, '2');
            break;
          case 'load':
            typeNode.textContent = 'l';
            typeNode.setAttribute(`data-color-index`, '3');
            break;
          case 'execute':
            typeNode.textContent = 'x';
            typeNode.setAttribute(`data-color-index`, '4');
            break;
          case 'validate':
