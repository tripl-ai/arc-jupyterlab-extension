...
import { ReadonlyJSONObject, JSONArray, JSONValue, JSONObject } from '@lumino/coreutils';
  ...
  private _onReply(
    ...
    if (types) {
      types.forEach((item: JSONValue) => {
        ...
        const text = (<JSONObject>item).text as string;
        const type = (<JSONObject>item).type as string;
        ...
        if (matchSet.has(text)) {
          ...
          if (replaceText !== '<unknown>') {
            replaceMap[text] = replaceText;
          }
          if (language !== '<unknown>') {
